...
//echo $message;die("here");
                ...
                $subject = "Reset Your Password: Valogical";
                ...
                if(mail($to2,$subject,$message,$headersad) && mail($to1,$subject,$message,$headersad))  {
      					$this->session->set_flashdata('success_msg', 'Please confirm the request to reset your password by clicking the link sent to your email address.');
                           ...
                           redirect('client/resetpassword');

                }
				}else{

                  $this->session->set_flashdata('err_msg', 'The given email is not exists.');
    ...
    { 
        ...
        $check=$this->Reset_model->check_user($hprk); 
         ...
         if($check){
          ...
          $_SESSION['cust_code']=$check->customer_id_fk;
                ...
                $this->form_validation->set_rules('confirm_password', 'Confirm Password ', 'trim|required|matches[new_password]');
...

                  {
                    ...
                    $data['password']= md5($this->input->post('new_password'));
                      ...
                      $cust_code=$this->input->post('user_code');

                      //echo "$cust_id"; die("hbv hjb");

                      $update=$this->Reset_model->update_password($data,$cust_code);

                      $update_status=$this->Reset_model->update_status($cust_code);
