		...
		{	
			...
			$this->form_validation->set_rules('preference', 'Preference', 'trim|required');
             ...
             $this->load->library('upload');
                ...
                $email_check=$this->Customer_model->email_check($data_user['email']);
                if($email_check)
                {
                    $data_inserted_customer = $this->Customer_model->add_customer($data_customer);
	                $data_inserted_users = $this->Customer_model->add_user($data_user);
	                $message="";
               ...
               // mail($to2,$subject,$message,$headersad) && mail($to1,$subject,$message,$headersad)
	                ...
	                if($data_inserted_customer && $data_inserted_users)  {
	                   // echo $message;die();
		...
		{	
			...
			$this->form_validation->set_rules('preference', 'Preference', 'trim|required');
             ...
             $this->load->library('upload');
                ...
                $data_updated_customer = $this->Customer_model->edit_customer($data_customer,$cust_code);
                $data_updated_users = $this->Customer_model->edit_users($data_user,$cust_code);
                /*print_r($data_updated_customer);die("here");
